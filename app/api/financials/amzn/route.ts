...
async function avFetch(fn: string, symbol: string, apikey: string) {
  ...
  const res = await fetch(url, {
    next: { revalidate: 60 * 60 * 24 * 7 }, // cache once a week
  });
  ...
  if (json?.Note || json?.Information || json?.Error_Message) {
    throw new Error(
      `AlphaVantage error: ${
        json?.Error_Message ?? json?.Information ?? json?.Note
      }`
    );
  }
...
function computeOpNwcFromBalanceSheet(bs: any) {
  ...
  const opAssets =
    totalCurrentAssets - (cash ?? 0) - (shortTermInvestments ?? 0);
...
export async function GET() {
  try {
    ...
    const annualIS = Array.isArray(isJson?.annualReports)
      ? isJson.annualReports
      : [];
    const annualBS = Array.isArray(bsJson?.annualReports)
      ? bsJson.annualReports
      : [];
    const annualCF = Array.isArray(cfJson?.annualReports)
      ? cfJson.annualReports
      : [];
    ...
    const annual = dates.map((end: string) => {
      ...
      const ebitMargin =
        revenue && ebit !== null ? ebit / revenue : null;
    ...
    for (let i = 0; i < annual.length; i++) {
      ...
      curr.deltaOpNwc =
        curr?.opNwc != null && prev?.opNwc != null
          ? curr.opNwc - prev.opNwc
          : null;
    }
