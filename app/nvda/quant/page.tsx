...
export default async function NvdaQuantPage() {
    ...
    return (
      <main className="min-h-screen bg-zinc-50">
        <div className="p-10 max-w-5xl mx-auto space-y-6">
          <h1 className="text-4xl font-semibold text-zinc-900">NVDA</h1>
  ...
  const muD = mean(logRets); // daily drift estimate (historical)
  const sigD = std(logRets); // daily vol estimate

  // Monte Carlo settings
  const horizonDays = 252; // 1Y
  const nSims = 5000;
  const lambda = 0.75; // risk aversion penalty for "risk-adjusted price"

  // simulate terminal prices under GBM: S_T = S0 * exp((mu - 0.5*s^2)T + s*sqrt(T)*Z)
  const rng = mulberry32(42);
  const T = horizonDays; // measured in trading days
  ...
  // returns distribution for VaR/CVaR
  const terminalRets = terminalPrices.map((x) => x / S0 - 1);
  const retSorted = [...terminalRets].sort((a, b) => a - b);
  const var05 = quantile(retSorted, 0.05); // 5% VaR on returns
  const cvar05 = mean(retSorted.filter((r) => r <= var05)); // average of worst 5%

  // simple "risk-adjusted" target: mean - lambda * std
      ...
      <div className="p-10 max-w-5xl mx-auto space-y-6">
        ...
        <div className="mt-4 border-b border-zinc-200">
          <nav className="flex gap-4" aria-label="tabs">
            <Link
              href="/nvda"
              className="pb-3 text-zinc-600 hover:text-zinc-900"
            >
              Overview
            </Link>
            <Link
            ...
            <Link
              href="/nvda/quant"
              className="pb-3 border-b-2 border-zinc-900 text-zinc-900 font-medium"
            >
              Quant
            </Link>
          </nav>
